...
from app.schemas.solve import SolveCreate, SolveResponse, SolveUpdate, StatsResponse
...
async def get_user_stats(
    ...
    stmt = select(Solve).where(Solve.user_id == current_user.id).order_by(Solve.created_at.asc())
    result = await db.execute(stmt)
    solves = result.scalars().all()
